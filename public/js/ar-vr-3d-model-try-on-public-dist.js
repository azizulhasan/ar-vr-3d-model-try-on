              ...
              return (0,_src_context_utilities__WEBPACK_IMPORTED_MODULE_1__.postWithoutImage)((0,_src_context_utilities__WEBPACK_IMPORTED_MODULE_1__.getURL)('get_model_and_settings'), _formData).then(function (response) {
                ...
                if (response.success) {
                  ...
                  if (response.success) {
                    ...
                    // Use the product name as the modal title
                    var _productName = _data.product_name || '3D Product';
                    alertifyjs__WEBPACK_IMPORTED_MODULE_0___default().alert(_productName, _htmlContent).set({
                      transition: 'zoom',
                      movable: true,
                      maximizable: true
                    }) // Customize options
                    .setHeader(_productName);

                    // Check if the data exists before assigning it to model-viewer
                    if (_data) {
                      var _modelViewer = document.getElementById('model-viewer');
                      if (_modelViewer) {
                        _modelViewer.setAttribute('src', _data.model_3d_file || '');
                        _modelViewer.setAttribute('alt', _data.model_alt || '');
                        _modelViewer.setAttribute('poster', _data.model_poster || '');
                        _modelViewer.setAttribute('reveal', _data.reveal || 'auto');
                        _modelViewer.setAttribute('loading', _data.loading || 'auto');
                        _modelViewer.setAttribute('ar-modes', (_data.ar_modes || []).join(' '));
                        _modelViewer.style.backgroundColor = _data.poster_color || 'rgba(255,255,255,0)';
                        var _scale = _data.scale || 'auto'; // Default value if not defined
                        _modelViewer.setAttribute('ar-scale', _scale); // Use "auto" or "fixed" as needed
                      }
                    }
                  } else {
                    console.error(response.data);
